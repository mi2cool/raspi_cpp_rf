...
bool RECEIVE_ATTR RCSwitch::receiveProtocol(const int p, unsigned int changeCount)
{
  ...
  for (unsigned int i = 1; i < changeCount - 1; i += 2)
  {
    ...
    else
    {
      ...
      printf("delay: %i\n", delay);
      printf("delayTolerance: %i\n", delayTolerance);
      printf("RCSwitch::timings[i]: %i\n", RCSwitch::timings[i]);
      printf("pro.zero.low: %i\n", pro.zero.low);
      printf("pro.zero.high: %i\n", pro.zero.high);
      printf("pro.syncFactor.low;: %i\n", pro.syncFactor.low);
      printf("diff(RCSwitch::timings[i], delay * pro.zero.high): %i\n", diff(RCSwitch::timings[i], delay * pro.zero.high));
      printf("diff(RCSwitch::timings[i + 1], delay * pro.zero.low): %i\n", diff(RCSwitch::timings[i + 1], delay * pro.zero.low));
      return false;
    }
  }
...
void RECEIVE_ATTR RCSwitch::handleInterrupt()
{
  ...
  if (duration > RCSwitch::nSeparationLimit)
  {
    ...
    // printf("diff(duration, RCSwitch::timings[0]) %i\n", diff(duration, RCSwitch::timings[0]));
    // printf("RCSwitch::nSeparationLimit %i\n", RCSwitch::nSeparationLimit);

    if (diff(duration, RCSwitch::timings[0]) < 200)
    {
      // printf("duration, RCSwitch::timings[0]) < 200 --> true\n");
      ...
      if (repeatCount == 2)
      {
        for (unsigned int i = 1; i <= numProto; i++)
        {
          if (receiveProtocol(i, changeCount))
          {
            ...
            printf("receive succeeded for protocol i");
